...
//    virtual void                                  set_tensor( std::shared_ptr< Kernel > )        override{};
  ...
  WeightsTransposedConvolution< T, K, Alps::Arch::CPU, A, S, D >::set_activations( std::vector< Alps::LayerTensors< T, D > >& Prev_image_tensors,
										    ...
										    std::vector< Alps::LayerTensors< T, D > >& Image_tensors )
  {
   ...
   //
    ...
    std::cout
      ...
      << " Prev_layer_size[ERROR]: " << Prev_image_tensors[0].get_image(TensorOrder1::ERROR).get_tensor_size()[0]
      << " Prev_layer_size[WERROR]: " << Prev_image_tensors[0].get_image(TensorOrder1::WERROR).get_tensor_size()[0]
      << " layer_size[ACTIVATION]: " << Image_tensors[0].get_image(TensorOrder1::ACTIVATION).get_tensor_size()[0]
      << " layer_size[ERROR]: " << Image_tensors[0].get_image(TensorOrder1::ERROR).get_tensor_size()[0]
      << " layer_size[WERROR]: " << Image_tensors[0].get_image(TensorOrder1::WERROR).get_tensor_size()[0]
      << " size_in: " << size_in
	...
	for (int k = 0 ; k < size_in ; ++k )
	  {
	    ...
	    std::cout
	      << "Prev_tensors["<<f<<"][WERROR]["<<k<<"] = " << Prev_image_tensors[f][TensorOrder1::WERROR][k]
	      << std::endl;
	  }
      }
  };
